...
try {
    ...
    $stmt = $conn->prepare("SELECT id FROM usuarios WHERE id = ? AND rol = 'afiliado'");
    ...
    if ($resultado) {
        ...
        sendResponse(['success' => false, 'error' => 'Error al actualizar afiliado'], 500);
    }

} catch (Exception $e) {
    error_log('Error editando afiliado: ' . $e->getMessage());
    sendResponse([
        'success' => false, 
        'error' => 'Error al editar afiliado: ' . $e->getMessage()
    ], 500);
}
?>